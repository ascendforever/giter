...
class _SuperIteratorMeta(type):
    ...
    def __instancecheck__(cls, instance) -> bool:
        ...
        for k,v in cls._mapping_.items():
            if _any(clss is cls for clss in k) and _any(_type___instancecheck__(c, instance) for c in v): # might wan't to do THIS check here if this doesn't already work (rather than the type. version)
                return True
    ...
    def __subclasscheck__(cls, subclass) -> bool:
        ...
        _any = any
        for k,v in cls._mapping_.items():
            if _any(c is cls for c in k) and _any(_type___subclasscheck__(c, instance) for c in v): # might wan't to do THIS check here if this doesn't already work (rather than the type. version)
                return True
